            ...
            if (Request.Cookies["CartPID"] != null)
            {
                ...
                if (CookieDataArray.Length > 0)
                {
                    ...
                    for (int i = 0; i < CookieDataArray.Length; i++)
                    {
                        ...
                        {
                            using (SqlCommand cmd = new SqlCommand("select A.*,dbo.getSizeNamew(" + SizeID + ") as SizeNamee,"
            ...
            if (Session["EMAIL"] != null)
            {
                ...
                using (SqlConnection con = new SqlConnection(CS))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("insert into tblPurchase values('" + USERID + "','"
                        ...
                        + txtName.Text + "','" + txtAddress.Text + "','" + txtemail.Text + "','" + txtMobileNumber.Text + "') select SCOPE_IDENTITY()", con);
                    int t = cmd.ExecuteNonQuery();
                    if (t > 0)
                    {

                        Response.Write("<script> alert('Congrats! Your Order Placed Successfully ');  </script>");
